...
function addEvents(){
    ...
    document.querySelector('#checkout .cart-btn').addEventListener('click', function() {
    ...
    // Redirect to Your Orders section/page
    // If using single page app with section toggle:
    showSection('orders'); // IDs must match your sections

    // Or, if "Your Orders" is a separate page, use:
...
heart.innerText = 'Add to Wishlist';
heart.onclick = function() { addToWishlist(item.id); };
itemCard.appendChild(heart);
let wishlistData = [];

// Adds an item to wishlist and avoids duplicates
function addToWishlist(itemId) {
  const item = foodItem.find(f => f.id === itemId);
  if (!wishlistData.some(f => f.id === itemId)) {
    wishlistData.push({...item});
    alert(item.name + ' added to your wishlist!');
    // Update wishlist section if open
...
// Render wishlist items in wishlist section
function renderWishlist() {
  ...
  wishlistData.forEach(item => {
    ...
    div.innerHTML =
      `<img src="${item.img}" alt="${item.name}" style="width:80px;margin-right:10px;vertical-align:middle;border-radius:8px;">
       <span style="font-size:1.1em;">${item.name} ($${item.price})</span>
       <button onclick="removeFromWishlist(${item.id})" style="margin-left:12px;background:#ea572a;color:#fff;border:none;border-radius:5px;padding:5px 10px;cursor:pointer;">Remove</button>`;
...
function removeFromWishlist(itemId) {
  wishlistData = wishlistData.filter(f => f.id !== itemId);
  renderWishlist();
}

// Update showSection function to render wishlist on open
function showSection(section) {
  document.querySelectorAll('.section-content').forEach(sec => sec.style.display = 'none');
  document.querySelectorAll('.sidebar a').forEach(link => link.classList.remove('active'));
  document.getElementById(section + '-section').style.display = 'block';
  document.getElementById(section + '-link').classList.add('active');
  // If 'wishlist' is opened, render wishlist items
